...
try {
    Start-Process "uplay://launch/${GameID}" -ErrorAction Stop
...
try {
    ...
    Stop-Process -Name "upc" -Force -ErrorAction Stop
    Stop-Process -Name "UbisoftGameLauncher" -Force -ErrorAction Stop
    Stop-Process -Name "UplayWebCore" -Force -ErrorAction Stop
}
catch {
